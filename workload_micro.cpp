...
//typedef int32_t keytype; /* default: uint64_t */
//typedef std::less<int32_t> keycomp;
...
inline void exec(int num_thread)
{
    ...
    auto func_unit = [idx, num_insert](uint64_t thread_id, bool)
    {
        ...
        {
            uint64_t random_value = std::rand() % 1000 + 1;
            idx->insert(random_value, random_value, ti);
        }
    };

    start_time = get_now();
    StartThreads(idx, num_thread, func_unit, false);
    end_time = get_now();

    std::cout << std::fixed;
    tput = num_insert / (end_time - start_time);
    elapsed_time = (end_time - start_time);
